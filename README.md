...
- **Quantum execution via QAOA** (`backend/app/optimization/qaoa.py` and `backend/compare_solvers.py`) - The QUBO is directly portable to quantum platforms. We include a solver wrapper executing the QAOA algorithm on Qiskit Aer's local simulator. The live backend uses a **dual-solver routing architecture** (SolverRegistry): it safely runs QAOA for small benchmarks (<= 16 qubits) while gracefully bypassing the memory-heavy simulator and falling back to classical SA for live production datasets (e.g. 88 variables) to guarantee lightning-fast demo times.
...
**Demo credentials:** `treasury@demo-bank.com` / `DemoPassword123!` (synthetic demo account only).
...
> **Honesty note on Docker.** The Dockerfiles and `docker-compose.yml` in this repo are complete and were reviewed for correctness, but they were **not** executed inside the sandbox this project was built in (no Docker daemon / container registry access there). Every other claim in this README - the optimization run, the tests, the frontend build - was actually executed and its real output is shown above. Please run `docker compose up --build` once yourself before a live demo.
...
| 15 frontend pages | 9 pages | Login, Dashboard, Corridors, Optimizer, QUBO Inspector, Scenarios, Stress Tests, Agent, Audit - the pages that carry the actual story |
...
| LLM-backed agent | Deterministic agent, LLM phrasing hook present but untested (no key here) | Per the spec's own §44: the system must never fail without an LLM key. The deterministic path is the tested, default path |
| `docker compose up` executed end-to-end | Dockerfiles/compose written and reviewed, tested via local SQLite+uvicorn+vite instead | No Docker daemon in the build sandbox |
