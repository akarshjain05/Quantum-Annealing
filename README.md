...
- **Quantum execution via QAOA** (`backend/app/optimization/qaoa.py` and `backend/compare_solvers.py`) - The QUBO is directly portable to quantum platforms. We include a solver wrapper executing the QAOA algorithm on Qiskit Aer's local simulator, along with a direct benchmarking script comparing QAOA vs. Classical SA vs. Brute-Force for Track 2 requirements.
