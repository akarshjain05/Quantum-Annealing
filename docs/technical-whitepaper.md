...
Every run requires an explicit Approve / Reject / Request Recalculation decision, logged with actor, timestamp, and reason (`api/optimization.py::approve_run`). The UI states plainly that this is a decision-support system and no live financial transaction is executed.
