...
def _handle_general_snapshot(db: Session, question: str) -> tuple:
    tools_used = ["get_liquidity_snapshot"]
    ...
    return text_parts, tools_used, []

def _handle_excess(db: Session, question: str) -> tuple:
    ...
    if lines:
        text_parts = ["Top candidates for capital release:"] + ["- " + l for l in lines]
    else:
        text_parts = ["No significant excess liquidity found in the analyzed corridors."]
    return text_parts, tools_used, []

def _handle_scenario_demand(db: Session, question: str) -> tuple:
...
def _handle_scenario_volatility(db: Session, question: str) -> tuple:
    tools_used = ["get_payment_forecast", "run_optimizer", "run_scenario"]
    corridor = _find_corridor(db, question)
    pct_match = PERCENT_RE.search(question)
    pct = float(pct_match.group(1)) if pct_match else 20.0
...
def _handle_scenario_confidence(db: Session, question: str) -> tuple:
...
def _handle_scenario_cutoff(db: Session, question: str) -> tuple:
    ...
    return text_parts, tools_used, sources

def _handle_source_regulation(db: Session, question: str) -> tuple:
    tools_used = ["get_regulatory_constraints"]
...
def _handle_source_practice(db: Session, question: str) -> tuple:
    tools_used = ["get_settlement_practices"]
    practices = agent_tools.get_settlement_practices(db)
    text_parts, sources = ["Observed settlement/operational practice items (not formal regulation):"], []
    for p in practices:
...
def _handle_binding_constraint(db: Session, question: str) -> tuple:
...
_INTENT_HANDLERS = {
    "general_snapshot": _handle_general_snapshot,
    "largest_excess": _handle_excess,
    "explain_excess": _handle_excess,
    "release_candidates": _handle_excess,
    "scenario_demand": _handle_scenario_demand,
    "scenario_volatility": _handle_scenario_volatility,
    "scenario_confidence": _handle_scenario_confidence,
    "scenario_cutoff": _handle_scenario_cutoff,
    "source_regulation": _handle_source_regulation,
    "source_practice": _handle_source_practice,
    "binding_constraint": _handle_binding_constraint,
}
...
def answer_question(db: Session, question: str) -> Dict[str, Any]:
    ...
    if handler:
        text_parts, tools_used, sources = handler(db, question)
    else:
        text_parts = ["I didn't confidently match that to a known question type - try asking about excess liquidity, a specific corridor scenario, or a regulation/practice lookup."]
        tools_used, sources = [], []
